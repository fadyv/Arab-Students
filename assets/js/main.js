...
let selectedOption = 'Bachelor';

function selectOption(option) {
    let showBachelorForm = document.getElementById('show-bachelor-form');
    let showMasterForm = document.getElementById('show-master-form');
    let showPhdForm = document.getElementById('show-phd-form');

    // Reset display style for all forms
    showBachelorForm.style.display = 'none';
    showMasterForm.style.display = 'none';
    showPhdForm.style.display = 'none';

    // Reset required attribute for all inputs in all forms
    resetRequiredValidation(showBachelorForm.getElementsByTagName('input'));
    resetRequiredValidation(showMasterForm.getElementsByTagName('input'));
    resetRequiredValidation(showPhdForm.getElementsByTagName('input'));

    // Set display style and required attribute based on the selected option
    switch (option) {
        case 'Bachelor':
            showBachelorForm.style.display = 'block';
            setRequiredValidation(showBachelorForm.getElementsByTagName('input'));
            break;

        case 'Master':
            showMasterForm.style.display = 'block';
            setRequiredValidation(showMasterForm.getElementsByTagName('input'));
            break;

        case 'PhD':
            showPhdForm.style.display = 'block';
            setRequiredValidation(showPhdForm.getElementsByTagName('input'));
            break;
...
function resetRequiredValidation(inputs) {
    for (let i = 0; i < inputs.length; i++) {
        inputs[i].removeAttribute('required');
    }
}

function setRequiredValidation(inputs) {
    for (let i = 0; i < inputs.length; i++) {
...
$(document).ready(function(){
  ...
  if ( $('#show-bachelor-form').length > 0   ){
       selectOption(selectedOption); // addmission form
       var selectElement = document.getElementById('degreeNeeded');
